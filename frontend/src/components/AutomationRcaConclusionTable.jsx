...
function AutomationRcaConclusionTable({ sourceResponse }) {
    ...
    fetch(endpoint, {
      ...
      .catch((loadError) => {
        if (loadError.name !== 'AbortError') {
          setRows([])
          setError(loadError.message || 'RCA conclusion counts could not be loaded.')
        }
      })
      .finally(() => {
  ...
  return (
    <section className="summary-section" aria-labelledby="automation-rca-heading">
      <div className="summary-heading">
        <div>
          <p className="section-label">Automation Rca Conclusion</p>
          <h3 id="automation-rca-heading">Automation Rca Conclusion counts</h3>
        </div>
      ...
      {isLoading ? (
        <p className="empty-results">Loading {period} RCA conclusion counts...</p>
      ) : rows.length > 0 ? (
        ...
        <p className="empty-results">No RCA conclusion count data was returned for this period.</p>
      )}
      {rows.length > 0 && (
        <div className="pagination-controls" aria-label="RCA conclusion pagination">
