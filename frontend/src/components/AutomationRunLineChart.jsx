...
function AutomationRunLineChart({ sourceResponse }) {
  ...
  useEffect(() => {
    ...
    cacheRef.current = {
      monthly: formatData(calculatePeriodSummary(rawRows, 'monthly', 'automation_run')),
      weekly: formatData(calculatePeriodSummary(rawRows, 'weekly', 'automation_run')),
      daily: formatData(calculatePeriodSummary(rawRows, 'daily', 'automation_run')),
    }
  ...
  useEffect(() => {
    ...
    if (sourceResponse && Array.isArray(sourceResponse.rows)) {
      const summary = calculatePeriodSummary(sourceResponse.rows, period, 'automation_run')
